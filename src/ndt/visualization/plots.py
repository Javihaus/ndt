...
def plot_single_metric(
    ...
    if show_loss and 'loss' in df.columns:
        ax2 = ax.twinx()
        ax2.plot(df['step'], df['loss'], linewidth=1, alpha=0.6,
                label='Loss', color='coral', linestyle='--')
        ax2.set_ylabel('Loss', fontsize=12, color='coral')
        ax2.tick_params(axis='y', labelcolor='coral')

    ax.set_title(f'{metric.replace("_", " ").title()} - {layer_name}', fontsize=14, fontweight='bold')
    fig.tight_layout()
...
def plot_metrics_comparison(
    ...
    for ax, metric in zip(axes, metrics):
        ...
        if ax == axes[-1] and 'loss' in df.columns:
            ax2 = ax.twinx()
            ax2.plot(df['step'], df['loss'], linewidth=1, alpha=0.4,
...
def plot_phases(
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} Across Layers', fontsize=14, fontweight='bold')
...
def plot_jumps(
    ...
    if jumps:
        ...
        jump_values = [df[df['step'] == j.step][metric].values[0] if j.step in df['step'].values
                      else j.value_after for j in jumps]

        ax.scatter(jump_steps, jump_values, color='red', s=100, zorder=5,
                  label=f'Jumps (n={len(jumps)})', marker='o', edgecolors='darkred', linewidths=2)
